...
import type { DirectoryAccount, DirectoryHome, DirectoryHomeMembership } from '../domain/entities.js';
...
export class DirectoryService {
  constructor(private readonly store: DirectoryStore, private readonly invitationTtlMs = 7 * 24 * 60 * 60 * 1000) {}
  async registerAccount(input: { email: string; displayName: string; password: string }): Promise<DirectoryAccount> {
    ...
    const account: DirectoryAccount = { id: randomUUID(), email, displayName, passwordHash: await bcrypt.hash(input.password, 12), createdAt: clockNow() };
    await this.store.createAccount(account); return account;
  }
  ...
  async registerHome(actorId: string, input: { name: string; edgeHostname: string }): Promise<DirectoryHome> {
  ...
  async invite(actorId: string, homeId: string, inviteeEmail: string): Promise<{ token: string; expiresAt: string }> {
    ...
    if(existing) await this.store.updateMembership(membership); else await this.store.createMembership(membership); await this.audit(actorId,homeId,membership.id,'membership.invited'); return {token,expiresAt};
  }
  ...
  private async requireAccount(id:string):Promise<void>{if(!await this.store.findAccountById(id)) throw new AuthenticationError('ACCOUNT_NOT_FOUND');}
