...
q('#pair-edge').onclick=async()=>{if(!state.selectedHome||state.selectedHome.role!=='owner')return;const output=q('#edge-credential');output.textContent='Generando código seguro…';try{const connection=await api(`/directory/homes/${state.selectedHome.id}/edge-connection`,{method:'POST'});output.textContent=`Gateway: ${connection.gatewayUrl}\nHogar: ${connection.homeId}\nEdge: ${connection.edgeId}\nCredencial: ${connection.token}`;notice('Entrega este paquete al instalador por un canal seguro. La credencial se muestra una sola vez.');}catch(error){output.textContent='';notice(error.message);}};q('#logout').onclick=()=>{localStorage.removeItem('directory_token');state.token=null;state.selectedHome=null;q('#homes').hidden=true;q('#auth').hidden=false;q('#logout').hidden=true;};document.querySelectorAll('[data-tab]').forEach(button=>button.onclick=()=>setTab(button.dataset.tab));
