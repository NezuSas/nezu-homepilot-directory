...
/** Transitional storage value for migrated Edge installations; never shown to users. */
export const UNPAIRED_EDGE_HOSTNAME = 'https://edge-unpaired.homepilot.invalid';
export function createHome(name: string, edgeHostname: string | undefined, ownerAccountId: string, now = clockNow()): DirectoryHome { return { id: randomUUID(), name: normalizeName(name, 'HOME_NAME_INVALID'), edgeHostname: normalizeEdgeHostname(edgeHostname ?? UNPAIRED_EDGE_HOSTNAME), ownerAccountId, createdAt: now, updatedAt: now }; }
